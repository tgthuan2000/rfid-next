                      ...
                      <tr className="cursor-pointer hover:opacity-70">
                        ...
                        <td
                          className={clsx(
                            ...
                            item.count > 0 ? "text-green-500" : "text-gray-500"
                          )}
                        >
                          {item.count ?? "-"}
                        </td>
...
const AllView = () => {
  ...
  const { data, loading, push, deleteAt, updatedAt } = useQuery(
    rfid.GET_MAPPING
  );

  useEffect(() => {
    const subscription = client
      .listen("*[_type == 'mapping']")
      .subscribe(async (data) => {
        ...
        if (result) {
          const res = await client.fetch(rfid.GETBY_MAPPING, {
            _id: documentId,
          });
          d = res[0];
        }
